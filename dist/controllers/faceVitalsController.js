...
import { listVitalsLinks, getVitalsLink, listScans, getScan, listTemplates, getTemplate, getScanUrl, formatMetrics, } from '../services/upvioVitals.js';
 ...
 */
export async function getTokenBalance(req, res, next) {
    try {
        const result = await pool.query('SELECT face_vitals_tokens FROM users WHERE id = $1', [req.user.userId]);
 ...
 */
export async function getScanLink(req, res, next) {
    try {
        // Check token balance
        const userResult = await pool.query('SELECT face_vitals_tokens FROM users WHERE id = $1', [req.user.userId]);
        ...
        // Deduct 1 token
        const updated = await pool.query('UPDATE users SET face_vitals_tokens = face_vitals_tokens - 1, updated_at = NOW() WHERE id = $1 RETURNING face_vitals_tokens', [req.user.userId]);
 ...
 */
export async function pollScanResults(req, res, next) {
    try {
        ...
        const succeeded = scans.find((s) => s.status === 'SUCCEEDED' && s.results?.metrics);
        if (!succeeded) {
            // Check if any are still in progress
            const inProgress = scans.find((s) => s.status === 'PENDING' || s.status === 'STARTED');
            res.json({
 ...
 */
export async function getFaceScan(req, res, next) {
    try {
 ...
 * List all scans
 */
export async function listFaceScans(req, res, next) {
    try {
        const linkId = req.query.linkId;
 ...
 */
export async function getFaceScanResults(req, res, next) {
    try {
        const scanId = String(req.params.scanId);
        const response = await getScan(scanId);
        const scan = response.data;
 ...
 */
export async function listBusinessVitalsLinks(req, res, next) {
    try {
 ...
 */
export async function getBusinessVitalsLink(req, res, next) {
    try {
        const businessId = String(req.params.businessId);
 ...
 */
export async function listBusinessVitalsScans(req, res, next) {
    try {
        const businessId = String(req.params.businessId);
 ...
 */
export async function getBusinessVitalsScan(req, res, next) {
    try {
        const businessId = String(req.params.businessId);
 ...
 */
export async function listBusinessVitalsTemplates(req, res, next) {
    try {
        const businessId = String(req.params.businessId);
 ...
 */
export async function getBusinessVitalsTemplate(req, res, next) {
    try {
        const businessId = String(req.params.businessId);
...
//# sourceMappingURL=faceVitalsController.js.map