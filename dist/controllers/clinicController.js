...
export async function getClinics(req, res, next) {
    try {
        const result = await pool.query('SELECT * FROM clinics WHERE is_active = true ORDER BY name');
        const rows = result.rows.filter((row) => !HIDDEN_CLINIC_NAMES.has(String(row.name || '').trim().toLowerCase()));
        res.json(rows);
    }
    catch (error) {
        next(error);
...
export async function getDoctorById(req, res, next) {
    try {
        const result = await pool.query(`SELECT d.*, c.name as clinic_name, c.location as clinic_location
...
export async function getSpecialties(req, res, next) {
    try {
        const result = await pool.query('SELECT DISTINCT specialty FROM doctors WHERE is_available = true ORDER BY specialty');
