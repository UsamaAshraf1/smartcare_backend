...
import { loginEmailSchema, loginOtpRequestSchema, loginOtpVerifySchema, registerSchema } from '../middleware/validators.js';
...
export async function register(req, res, next) {
    try {
        ...
        const result = await pool.query(`INSERT INTO users (name, email, password_hash, phone, emirates_id, passport_number, role, auth_method)
       VALUES ($1, $2, $3, $4, $5, $6, 'patient', 'email')
       RETURNING id, name, email, phone, emirates_id, passport_number, role, emr_id, created_at`, [data.name, data.email, passwordHash, data.phone, data.emiratesId || null, data.passportNumber || null]);
...
export async function loginEmail(req, res, next) {
    try {
        ...
        const result = await pool.query('SELECT id, name, email, phone, emirates_id, passport_number, emr_id, password_hash, role, photo_url, preferences FROM users WHERE email = $1 AND is_active = true', [data.email]);
...
export async function verifyOtp(req, res, next) {
    try {
        ...
        let result = await pool.query(`SELECT id, name, email, phone, emirates_id, passport_number, emr_id, role, photo_url, preferences
       FROM users
       WHERE regexp_replace(COALESCE(phone, ''), '\\s+', '', 'g') = $1`, [cleanPhone]);
        if (result.rows.length === 0) {
            // Auto-register
            result = await pool.query(`INSERT INTO users (name, phone, role, auth_method)
         VALUES ('New Patient', $1, 'patient', 'otp')
         RETURNING id, name, email, phone, emirates_id, passport_number, emr_id, role, photo_url, preferences`, [cleanPhone]);
        }
        const user = result.rows[0];
...
export async function loginUaePass(req, res, next) {
    try {
...
export async function uaePassCallback(req, res, next) {
    try {
        ...
        let result = await pool.query('SELECT id, name, email, phone, emirates_id, passport_number, emr_id, role, photo_url, preferences FROM users WHERE emirates_id = $1', [uaePassProfile.emiratesId]);
        if (result.rows.length === 0) {
            result = await pool.query(`INSERT INTO users (emirates_id, name, email, role, auth_method)
         VALUES ($1, $2, $3, 'patient', 'uaepass')
         RETURNING id, name, email, phone, emirates_id, passport_number, emr_id, role, photo_url, preferences`, [uaePassProfile.emiratesId, uaePassProfile.name, uaePassProfile.email]);
        }
        const user = result.rows[0];
...
export async function getMe(req, res, next) {
    try {
        const result = await pool.query(`SELECT id, name, email, phone, emirates_id, passport_number, emr_id, role, photo_url, date_of_birth,
