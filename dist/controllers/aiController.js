...
export async function runEMRAssessment(req, res, next) {
    try {
        // Fetch patient's medical records
        const records = await pool.query('SELECT * FROM medical_records WHERE patient_id = $1 AND is_active = true', [req.user.userId]);
        if (records.rows.length === 0) {
            throw new AppError('No medical records found for assessment', 404);
        }
        // Build context from records
        const meds = records.rows.filter((r) => r.record_type === 'medication').map((r) => r.data);
        const labs = records.rows.filter((r) => r.record_type === 'lab_result').map((r) => r.data);
        const vitals = records.rows.filter((r) => r.record_type === 'vital').map((r) => r.data);
        const diagnoses = records.rows.filter((r) => r.record_type === 'diagnosis').map((r) => r.data);
    ...
    Provide actionable tips for nutrition, lifestyle, and include a 'Safety Escalation' section if any values are concerning.`;
        ...
        // Save assessment
        await pool.query(`INSERT INTO ai_assessments (patient_id, assessment_type, input_data, result, model)
       VALUES ($1, 'emr', $2, $3, 'gemini-2.0-flash')`, [req.user.userId, JSON.stringify({ meds, labs, vitals, diagnoses }), result]);
...
export async function runDocumentAnalysis(req, res, next) {
    try {
    ...
    recommended actions, and any values that need attention:\n\n${documentText}`;
        const result = await callGemini(prompt, SYSTEM_INSTRUCTION);
        await pool.query(`INSERT INTO ai_assessments (patient_id, assessment_type, input_data, result, model)
       VALUES ($1, 'document', $2, $3, 'gemini-2.0-flash')`, [req.user.userId, JSON.stringify({ documentLength: documentText.length }), result]);
...
export async function runVitalsAssessment(req, res, next) {
    try {
    ...
    Provide an immediate assessment and whether the patient should seek urgent care.`;
        const result = await callGemini(prompt, SYSTEM_INSTRUCTION);
        await pool.query(`INSERT INTO ai_assessments (patient_id, assessment_type, input_data, result, model)
       VALUES ($1, 'vitals', $2, $3, 'gemini-2.0-flash')`, [req.user.userId, JSON.stringify({ heartRate, bloodPressure, spO2, respiratoryRate, stressLevel }), result]);
...
export async function getAssessmentHistory(req, res, next) {
    try {
        const result = await pool.query('SELECT id, assessment_type, created_at FROM ai_assessments WHERE patient_id = $1 ORDER BY created_at DESC LIMIT 20', [req.user.userId]);
...
export async function getAssessmentById(req, res, next) {
    try {
        const result = await pool.query('SELECT * FROM ai_assessments WHERE id = $1 AND patient_id = $2', [req.params.id, req.user.userId]);
