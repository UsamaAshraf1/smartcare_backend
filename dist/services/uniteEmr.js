...
function requireUniteEnv(primaryKey, fallbackKey) {
    ...
    if (!value) {
        throw new Error(`[Unite EMR] Missing required env var: ${primaryKey} (or fallback ${fallbackKey})`);
    }
    return value;
}
const UNITE_BASE_URL = process.env.UNITE_EMR_BASE_URL ||
    process.env.UNITE_APPOINTMENT_BASE_URL;
if (!UNITE_BASE_URL) {
    throw new Error('[Unite EMR] Missing required env var: UNITE_EMR_BASE_URL (or fallback UNITE_APPOINTMENT_BASE_URL)');
}
 ...
 */
export async function getMedicalRecords(fromDate, toDate) {
 ...
 */
export function transformPatientData(rawPatient) {
    return {
        ...
        allergies: (rawPatient.allergy || []).map((a) => ({
            type: a.type || '',
            severity: a.severity || '',
            notes: a.notes || '',
        ...
        visits: (rawPatient.visitdetails || []).map((v) => ({
            date: v.visitdate || '',
            doctor: v.doctorname || '',
            ...
            vitals: v.vitals || [],
            diagnoses: (v.diagnosis || []).map((d) => ({
                type: d.type || '',
                code: d.code || '',
                description: d.description || '',
            })),
            medications: (v.medication || []).map((m) => ({
                code: m.localcode || '',
                name: m.name || '',
            ...
            items: Object.entries(v.items || {}).reduce((acc, [key, val]) => {
 ...
 */
export function formatDateForUnite(date) {
 ...
 */
export async function getRecentRecords(days = 30) {
    const safeDays = Math.max(1, Math.min(Number(days) || 30, 30));
    const toDate = new Date();
    const fromDate = new Date();
    fromDate.setDate(fromDate.getDate() - safeDays);
    const raw = await getMedicalRecords(formatDateForUnite(fromDate), formatDateForUnite(toDate));
...
//# sourceMappingURL=uniteEmr.js.map