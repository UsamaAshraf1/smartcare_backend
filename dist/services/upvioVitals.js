...
function resolveBusinessId(businessIdOverride) {
...
function businessUrl(path, businessIdOverride) {
    const businessId = resolveBusinessId(businessIdOverride);
    return `${UPVIO_BASE_URL}/businesses/${businessId}${path}`;
}
async function parseUpvioError(response, operation, businessIdOverride) {
    const err = await response.json().catch(() => ({ error: 'Unknown error' }));
    const businessId = resolveBusinessId(businessIdOverride);
    const title = err?.error?.title || err?.message || '';
    if (response.status === 400 && title === 'Business ID does not match API key') {
        throw new Error(`Upvio ${operation} failed (${response.status}): Business ID "${businessId}" does not match the configured API key. ` +
...
// ─── API Methods ──────────────────────────────────────────
export async function listVitalsLinks(params, businessId) {
    ...
    if (params?.vitalsTemplateId)
        query.set('vitalsTemplateId', params.vitalsTemplateId);
...
export async function getVitalsLink(linkId, businessId) {
...
export async function listScans(params, businessId) {
    const query = new URLSearchParams();
    if (params?.limit)
        query.set('limit', String(params.limit));
    if (params?.offset)
    ...
    if (params?.vitalsLinkId)
        query.set('vitalsLinkId', params.vitalsLinkId);
    const qs = query.toString();
...
export async function getScan(scanId, businessId) {
...
export async function getTemplate(templateId, businessId) {
...
// ─── Helpers ──────────────────────────────────────────────
export function getScanUrl(slug) {
...
//# sourceMappingURL=upvioVitals.js.map