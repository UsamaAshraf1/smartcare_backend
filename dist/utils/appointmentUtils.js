...
export function toHhMm24(input) {
...
export function addMinutesToHhMm(startTime, minutesToAdd) {
...
export function formatPhoneForUnite(input) {
...
export function normalizeAppointmentStatus(value) {
    ...
    if (!s)
        return 'confirmed';
    if (s === 'cvi' || s === 'cancelled' || s === 'canceled' || s.includes('cancel'))
        return 'cancelled';
    if (s === 'nsw' || s.includes('no show') || s === 'no_show')
        return 'no_show';
    if (s === 'aac' || s === 'acf' || s === 'aph' || s === 'ytc' || s.includes('confirm') || s.includes('honour'))
        return 'confirmed';
    return s;
}
// ── Unite response extraction ────────────────────────────────────────────────
/** Case-insensitive field lookup on an object. */
export function getFieldCI(obj, keys) {
...
function uniteCandidates(payload) {
    return [
        payload?.Data,
        payload?.data,
...
export function extractUniteAppointmentRefs(payload) {
...
export function extractUniteAppointmentId(payload) {
...
export function extractUniteStatusCode(payload) {
    for (const obj of uniteCandidates(payload)) {
        const code = String(getFieldCI(obj, ['appointmentstatus', 'status', 'statuscode', 'status_code']) ?? '').trim();
        if (code)
            return code;
    }
    return '';
...
export function toUniteIdValue(input) {
    const raw = String(input ?? '').trim();
 ...
 */
export function extractUniteStartDateTimeParts(ua) {
    const pickString = (keys) => String(getFieldCI(ua, keys) ?? '').trim();
    const normDate = (v) => toIsoDate(v.replace(/\//g, '-'));
    const parseDuration = () => {
...
export function toUniteGenderCode(gender) {
...
export function toUnitePhotoType(identifierType) {
    if (identifierType === 'emirates_id')
        return 'EMIRATES_ID';
    if (identifierType === 'passport_number')
        return 'Passport';
    return undefined;
}
// ── Slot key for matching ────────────────────────────────────────────────────
export function slotKey(date, startTime, doctorId) {
...
export function toUniteArray(data) {
