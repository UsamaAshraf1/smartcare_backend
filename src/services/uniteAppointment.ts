...
function requireUniteEnv(primaryKey: string, fallbackKey: string): string {
  ...
  if (!value) {
    throw new Error(
      `[Unite Appointment] Missing required env var: ${primaryKey} (or fallback ${fallbackKey})`,
    );
  }
  return value;
}
...
const UNITE_BASE_URL =
  process.env.UNITE_APPOINTMENT_BASE_URL || process.env.UNITE_EMR_BASE_URL;
if (!UNITE_BASE_URL) {
  throw new Error(
    '[Unite Appointment] Missing required env var: UNITE_APPOINTMENT_BASE_URL (or fallback UNITE_EMR_BASE_URL)',
  );
}
...
interface TokenData {
  access_token: string;
  expires_in: number; // minutes (per docs examples)
...
function buildUrl(path: string, query?: Record<string, string | undefined>) {
  ...
  for (const [key, value] of Object.entries(query)) {
    if (value !== undefined && value !== null && String(value).length > 0) qp.set(key, String(value));
  }
...
function toIsoDateStr(input: string): string {
...
function parse12HourTime(input: string): { hour: number; minute: number } | null {
...
function to24HourHHmm(input: string): string {
  const twelve = parse12HourTime(input);
  if (!twelve) return String(input || '').trim();
  return `${String(twelve.hour).padStart(2, '0')}:${String(twelve.minute).padStart(2, '0')}`;
}

function addMinutesHHmm(hhmm: string, minutesToAdd: number): string {
...
function normalizeAvailableSlots(payload: any, requestedDateIso?: string): any[] {
  ...
  if (Array.isArray(payload)) {
    const rows = payload.map((slot: any) => {
    ...
    const rowsWithDate = rows.filter((r: any) => String(r?.date || '').trim().length > 0);
    if (rowsWithDate.length === 0) return rows;

    return rowsWithDate.filter((r: any) => String(r.date || '') === targetDate);
  }
...
async function authorize(): Promise<string> {
  ...
  if (!cachedToken.access_token) throw new Error('Unite authorize failed: missing access_token');
  return cachedToken.access_token;
}

async function refreshToken(): Promise<string> {
  if (!cachedToken) return authorize();
  ...
  for (const path of tryPaths) {
    ...
    if (response.ok && isSuccess(data)) {
      ...
      if (cachedToken.access_token) return cachedToken.access_token;
    }
  }
...
async function getAccessToken(): Promise<string> {
  ...
  if (Date.now() < expiresAt - 5 * 60 * 1000) return cachedToken.access_token;

  return refreshToken();
}

async function gatewayGet(path: string, query?: Record<string, string | undefined>) {
...
async function gatewayPost(path: string, payload: Record<string, any>) {
  const token = await getAccessToken();
  const response = await fetch(buildUrl(path), {
    method: 'POST',
    headers: {
 ...
 */
export async function getUniteAvailableSlots(clinicId: string, doctorId: string, date: string) {
...
export async function updateUniteAppointment(payload: Record<string, any>) {
  return gatewayPost('/gateway/updateappointment', payload);
}

/** POST /gateway/updateappointmentwithitemdetails */
export async function updateUniteAppointmentWithItems(payload: Record<string, any>) {
 ...
 */
export async function getUniteAppointments(clinicId: string, fromDate: string, toDate: string) {
...
export async function createUniteInvoice(payload: Record<string, any>) {
