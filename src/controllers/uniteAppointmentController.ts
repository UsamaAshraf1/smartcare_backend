...
import {
  getUniteClinics,
  getUniteDoctors,
  getUniteAvailableSlots,
  createUniteAppointment,
  getUniteAppointments,
} from '../services/uniteAppointment.js';
import {
  toIsoDate,
  toDdMmYyyy,
  toHhMm24,
  ...
  extractUnitePhone,
  extractUniteDoctorId,
  extractUniteRecordStatus,
  splitName,
  toUniteGenderCode,
  toUnitePhotoType,
  slotKey,
...
export async function getDoctors(req: Request, res: Response) {
  try {
    ...
    const matchesClinic = (doctor: any, target: string): boolean => {
      ...
      for (const list of [doctor?.clinics, doctor?.cliniclist, doctor?.clinic_list].filter(Boolean)) {
        if (Array.isArray(list)) {
          for (const item of list) {
            ...
            if (values.map((v) => String(v ?? '').trim()).includes(target)) return true;
          }
    ...
    const filtered = clinicId ? rows.filter((d: any) => matchesClinic(d, clinicId)) : rows;
...
export async function createAppointment(req: Request, res: Response) {
  try {
    ...
    const userResult = await pool.query(
      `SELECT emirates_id, passport_number, emr_id, name, email, phone, date_of_birth, gender
    ...
    if (user.emr_id) { patientIdentifier = user.emr_id; identifierType = 'emr_id'; }
    else if (user.emirates_id) { patientIdentifier = user.emirates_id; identifierType = 'emirates_id'; }
    else if (user.passport_number) { patientIdentifier = user.passport_number; identifierType = 'passport_number'; }
    else {
    ...
    const unitePayload: Record<string, any> = {
      firstname: firstName,
    ...
    const mappingResult = await pool.query(
      `INSERT INTO unite_appointment_mappings
 ...
 */
export async function syncUniteAppointmentStatusesForPatient(patientId: string): Promise<number> {
  const mappingsResult = await pool.query(
    `SELECT id, unite_appointment_id, clinic_id, doctor_id, date, status, start_time, end_time, raw_response
  ...
  const clinicGroups = new Map<string, { minDate: string; maxDate: string; rows: any[] }>();
  for (const m of mappings) {
    const clinicId = String(m.clinic_id || '').trim();
    const dateStr = toIsoDate(String(m.date || ''));
    if (!clinicId || !/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) continue;
    ...
    if (dateStr < group.minDate) group.minDate = dateStr;
    if (dateStr > group.maxDate) group.maxDate = dateStr;
  ...
  for (const [clinicId, group] of clinicGroups) {
    try {
      ...
      const apptInfoMap = new Map<string, any>();
      const slotCandidates = new Map<string, string[]>();
      const slotCandidatesNoDoc = new Map<string, string[]>();
      const dateDoctorCandidates = new Map<string, string[]>();
      const dateOnlyCandidates = new Map<string, string[]>();
      const phoneDoctorDateCandidates = new Map<string, string[]>();
      const phoneDateCandidates = new Map<string, string[]>();
      const phoneDoctorCandidates = new Map<string, string[]>();
      const phoneOnlyCandidates = new Map<string, string[]>();
      ...
      for (const ua of apptArray) {
        ...
        if (refs.appointmentno) apptInfoMap.set(refs.appointmentno, info);
      ...
      for (const mapping of group.rows) {
        ...
        for (const key of lookupKeys) {
          ...
          if (hit) { derivedId = key; info = hit; break; }
        }
        ...
        if (!info) {
          ...
          if (withDocCands.length === 1) derivedId = withDocCands[0];
          else if (!withDocCands.length && noDocCands.length === 1) derivedId = noDocCands[0];
          else if (sameDateDocCands.length === 1) derivedId = sameDateDocCands[0];
          else if (sameDateCands.length === 1) derivedId = sameDateCands[0];
          ...
          if (!derivedId && patientPhoneNormalized) {
            ...
            if (pddCands.length === 1) derivedId = pddCands[0];
            else if (!pddCands.length && pDateCands.length === 1) derivedId = pDateCands[0];
            else if (pdCands.length === 1) derivedId = pdCands[0];
            else if (!pdCands.length && pCands.length === 1) derivedId = pCands[0];
          }

          info = derivedId ? apptInfoMap.get(derivedId) : undefined;
        }

        if (!derivedId || !info) continue;
      ...
      console.warn('[Unite Sync] Clinic sync skipped', { clinicId, error: (error as any)?.message });
    }
  }

  // ── Cross-clinic recovery ──────────────────────────────────────────────────
  const unmatchedMappings = mappings.filter(
    (m) => String(m.unite_appointment_id || '').trim() && !matchedMappingIds.has(String(m.id)),
  );
  ...
  if (unmatchedMappings.length > 0) {
    try {
      ...
      const allClinicIds = Array.from(new Set(
        allClinicRows.map((c: any) => String(c?.clinic_id ?? c?.clinicid ?? c?.id ?? c?.code ?? '').trim()).filter(Boolean),
      ));
      ...
      console.warn('[Unite Sync] Cross-clinic recovery skipped', { error: (error as any)?.message });
    }
  }
...
export async function syncStatuses(req: Request, res: Response) {
  try {
