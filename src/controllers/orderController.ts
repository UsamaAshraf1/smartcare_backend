...
export async function createOrder(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO orders (patient_id, service_type, total_amount, details, is_home_collection, home_address, status, payment_status)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending_payment', 'unpaid')
       RETURNING *`,
      [req.user!.userId, data.serviceType, data.totalAmount, JSON.stringify(data.details), data.isHomeCollection, data.homeAddress]
    );
...
export async function processPayment(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const order = await pool.query(
      'SELECT * FROM orders WHERE id = $1 AND patient_id = $2 AND payment_status = $3',
      [data.orderId, req.user!.userId, 'unpaid']
    );
    ...
    if (data.savedCardId) {
      const card = await pool.query(
        'SELECT stripe_pm_id FROM saved_payment_methods WHERE id = $1 AND user_id = $2',
        [data.savedCardId, req.user!.userId]
      );
    ...
    const result = await pool.query(
      `UPDATE orders
       ...
       RETURNING *`,
      [data.orderId, req.user!.userId, data.paymentMethod, stripePaymentId]
    );
    ...
    if (orderData.service_type === 'appointment' && orderData.details?.appointmentId) {
      await pool.query(
        ...
        [orderData.details.appointmentId]
      );
    }
    ...
    if (orderData.service_type === 'face_vitals') {
      ...
      await pool.query(
        'UPDATE users SET face_vitals_tokens = face_vitals_tokens + $1, updated_at = NOW() WHERE id = $2',
        [tokensToAdd, req.user!.userId]
      );
    }
