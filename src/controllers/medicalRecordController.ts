...
export async function getMedications(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      "SELECT * FROM medical_records WHERE patient_id = $1 AND record_type = 'medication' AND is_active = true ORDER BY recorded_at DESC",
      [req.user!.userId]
    );
    res.json(result.rows);
  } catch (error) {
...
export async function getAllergies(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      "SELECT * FROM medical_records WHERE patient_id = $1 AND record_type = 'allergy' AND is_active = true",
      [req.user!.userId]
    );
    res.json(result.rows);
  } catch (error) {
...
export async function getDiagnoses(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      "SELECT * FROM medical_records WHERE patient_id = $1 AND record_type = 'diagnosis' AND is_active = true ORDER BY recorded_at DESC",
      [req.user!.userId]
    );
    res.json(result.rows);
  } catch (error) {
...
export async function getVitals(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      "SELECT * FROM medical_records WHERE patient_id = $1 AND record_type = 'vital' ORDER BY recorded_at DESC LIMIT 20",
      [req.user!.userId]
    );
    res.json(result.rows);
  } catch (error) {
...
export async function getLabResults(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      "SELECT * FROM medical_records WHERE patient_id = $1 AND record_type = 'lab_result' ORDER BY recorded_at DESC",
      [req.user!.userId]
    );
    res.json(result.rows);
  } catch (error) {
...
export async function syncFaceVitalsRecord(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const target: 'self' | 'other' = scanTarget === 'other' ? 'other' : 'self';
    ...
    const inserted = await pool.query(
      `INSERT INTO medical_records (patient_id, record_type, title, data, recorded_by, recorded_at)
       VALUES ($1, 'vital', $2, $3::jsonb, $1, NOW())
       RETURNING id, title, recorded_at`,
      [patientId, title, JSON.stringify(payload)]
    );
...
export async function syncUniteRecords(req: Request, res: Response, next: NextFunction) {
  try {
    const patientId = req.user!.userId;
    ...
    const userResult = await pool.query(
      'SELECT emr_id, emirates_id, passport_number, name FROM users WHERE id = $1',
      [patientId]
    );
    ...
    const target = emrRecords.find((p: any) => {
    ...
    const inserts: Array<{ recordType: string; title: string; data: any }> = [];
    ...
    try {
      ...
      for (const row of inserts) {
        await pool.query(
          ...
          [patientId, row.recordType, row.title, JSON.stringify(row.data)]
        );
      }
