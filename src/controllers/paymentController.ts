...
async function getOrCreateStripeCustomer(userId: string): Promise<string> {
  const user = await pool.query(
    'SELECT stripe_customer_id, email, name, phone FROM users WHERE id = $1',
    [userId]
  );
...
export async function savePaymentMethod(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM saved_payment_methods WHERE user_id = $1 AND stripe_pm_id = $2',
    ...
    const result = await pool.query(
      `INSERT INTO saved_payment_methods (user_id, stripe_pm_id, brand, last4, exp_month, exp_year, is_default)
...
export async function deletePaymentMethod(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const card = await pool.query(
      'SELECT stripe_pm_id, is_default FROM saved_payment_methods WHERE id = $1 AND user_id = $2',
      [id, req.user!.userId]
    );
