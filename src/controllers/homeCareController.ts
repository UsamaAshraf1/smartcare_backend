...
export async function getHomeVisits(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      `SELECT o.*, hvt.clinician_name, hvt.status as tracking_status,
...
export async function getHomeVisitTracking(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      `SELECT hvt.*, o.details, o.status as order_status
       FROM home_visit_tracking hvt
       JOIN orders o ON hvt.order_id = o.id
       WHERE hvt.order_id = $1 AND o.patient_id = $2`,
...
export async function getCaregiverPlans(_req: Request, res: Response) {
  res.json([
...
export async function createCaregiverJob(req: Request, res: Response, next: NextFunction) {
  try {
    const data = createCaregiverJobSchema.parse(req.body);

    const priceMap: Record<string, number> = { hourly: 85, shift: 450, monthly: 8500 };
    const price = priceMap[data.plan];

    // Create order first
    const order = await pool.query(
      `INSERT INTO orders (patient_id, service_type, total_amount, details, status, payment_status)
       VALUES ($1, 'caregiver', $2, $3, 'pending_payment', 'unpaid')
       RETURNING *`,
      [req.user!.userId, price, JSON.stringify(data)]
    );

    // Create caregiver job
    const job = await pool.query(
      `INSERT INTO caregiver_jobs (order_id, patient_id, plan, start_date, end_date, hours_per_day, tasks, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending_payment')
       RETURNING *`,
      [order.rows[0].id, req.user!.userId, data.plan, data.startDate, data.endDate, data.hoursPerDay, JSON.stringify(data.tasks)]
    );
...
export async function getCaregiverJobs(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await pool.query(
      `SELECT cj.*, o.total_amount, o.payment_status, o.status as order_status
